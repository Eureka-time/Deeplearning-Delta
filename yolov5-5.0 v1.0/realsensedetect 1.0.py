        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    ux=int((xyxy[0]+xyxy[2])/2) #计算像素坐标ux
                    uy=int((xyxy[1]+xyxy[3])/2) #计算像素坐标uy
                    camera_xyz = rs.rs2_deproject_pixel_to_point(depth_intrin, (ux, uy), dist)  # 计算相机坐标系的xyz
                    camera_xyz = np.round(np.array(camera_xyz), 3)  # 转成3位小数
                    camera_xyz = camera_xyz.tolist()
                    cv2.circle(color_image, (ux,uy), 4, (255, 255, 255), 5)#标出中心点
                    cv2.putText(color_image, str(camera_xyz), (ux+20, uy+10), 0, 0.5,
                                [225, 255, 255], thickness=1, lineType=cv2.LINE_AA)#标出坐标
                    demo = open('C:/Users/Yuki/Desktop/shuju.txt', 'a+')
                    print(camera_xyz, file=demo)
                    demo.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='data/images/a1.mp4', help='source')
